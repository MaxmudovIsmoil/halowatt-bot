...
<div class="space-y-6">
    @foreach($providers as $key => $p)
        @php
            ...
            $formId = "provider-form-{$key}";
        @endphp
        <x-card title="{{ $p['label'] }}">
            <x-slot name="actions">
                <label class="flex items-center gap-2 text-xs font-medium text-slate-600 dark:text-slate-300">
                    <input type="checkbox" name="active" value="1" form="{{ $formId }}" {{ $p['active'] ? 'checked' : '' }} class="form-checkbox">
                    Faol
                </label>
            </x-slot>

            <form id="{{ $formId }}" method="POST" action="{{ route('settings.ai.update', $key) }}" class="space-y-4">
                @csrf
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" name="model" value="{{ $p['model'] }}" class="form-input">
                ...
                <x-btn variant="primary" type="submit" size="sm">Saqlash</x-btn>
            </form>
        </x-card>
    @endforeach
</div>
